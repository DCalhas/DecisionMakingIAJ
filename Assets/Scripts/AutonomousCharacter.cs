...
namespace Assets.Scripts
{
    public class AutonomousCharacter : MonoBehaviour
    {
        ...
        public void Initialize(NavMeshPathGraph navMeshGraph, AStarPathfinding pathfindingAlgorithm)
        {
            ...
            this.AStarPathFinding.NodesPerFrame = 500;
        ...
        void Update()
        {
			if (Time.time > this.nextUpdateTime || this.GameManager.WorldChanged) {
				...
				if (this.MCTSActive) {
					this.MCTS.InitializeMCTSSearch ();
				} else {
            ...
            if (this.AStarPathFinding.InProgress)
			{
                ...
                if (finished && this.currentSolution != null)
                {
					...
					this.currentSmoothedSolution = AStarPathFinding.pathSmoothing(this.currentSolution);
		...
		private void UpdateMCTS()
		{
			if (this.MCTS.InProgress) {
				//choose an action using the MCTS process
			...
			this.ProcessedActionsText.text = "Max Depth: " + this.MCTS.MaxPlayoutDepthReached.ToString();

			if (this.MCTS.CurrentIterations >= this.MCTS.MaxIterations && this.MCTS.BestActionSequence.Count > 0)
			{

				var q = this.MCTS.BestFirstChild.Q / this.MCTS.BestFirstChild.N;
